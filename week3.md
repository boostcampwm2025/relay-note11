...
- 수행 결과
  ...
  <img width="946" height="764" alt="image" src="https://github.com/user-attachments/assets/1476ec85-fa7e-4be8-b571-f8c2a77a2d1f" />
...
다음은 내가 오늘 4시간 동안 학습할 키워드야. 4시간 뒤에는 학습한 내용으로 간단한 프로그램을 작성할 계획이야.
...
### 통신 방식
- [ ] Unicast, Broadcast, Muliticast
  - [ ] 각 방식을 비교해보자 
### 통신 구조
- [ ] Client-Server 구조 
# 요청
4시간동안 학습할 학습 플래너를 만들어줘. 적절한 시간 분배도 함께해주면 좋을 거 같아
</div>
</details>
<details>
...
좋아, 지금 목표는 4시간 동안 **네트워크와 관련된 CS 개념과 Node.js의 net.socket 라이브러리**를 학습하고,
그걸 바탕으로 간단한 프로그램을 만드는 거야.
그럼 학습 플랜을 다음처럼 짜보자:
...
1. 핵심 CS 네트워크 개념을 이해하고
2. Node.js에서 `net.Socket`을 사용해보며
3. 이를 바탕으로 작은 통신 프로그램을 작성할 준비를 한다.
...
| 시간                   | 주제                                  | 목표                                            | 학습 방식                              |
| -------------------- | ----------------------------------- | --------------------------------------------- | ---------------------------------- |
| **0:00\~0:30 (30분)** | **IP / TCP / UDP**                  | - 각 프로토콜이 어떤 역할을 하는지 구분<br>- 특징 및 차이점 파악      | - 계층 구조 그림 그리기<br>- 상황 예시 생각해보기    |
| **0:30\~1:00 (30분)** | **Unicast / Broadcast / Multicast** | - 각 방식이 언제 사용되는지 이해<br>- 장단점 비교               | - 예시 시나리오 직접 상상해보기<br>- 표로 정리      |
| **1:00\~1:40 (40분)** | **HTTP (요청/응답 구조)**                 | - HTTP 요청/응답 구조 파악<br>- 주요 헤더 필드와 용도 익히기      | - curl 등으로 직접 요청 보내보기<br>- 응답 분석   |
| **1:40\~2:10 (30분)** | **Client-Server 구조**                | - 구조적 특징 이해<br>- 역할 분리와 흐름 파악                 | - 다이어그램 그려보기<br>- 클라이언트/서버 역할 나눠보기 |
| **2:10\~3:00 (50분)** | **Node.js `net.Socket` 사용법**        | - TCP 소켓 연결 방식 이해<br>- 기본 예제 따라 해보기           | - 문서 읽고 코드 따라 하기<br>- 연결 실습        |
| **3:00\~3:20 (20분)** | **전체 복습 및 정리**                      | - 개념 연결하기<br>- 프로그램 구상 시작                     | - 그림, 표로 정리<br>- 키워드 퀴즈 풀어보기       |
| **3:20\~4:00 (40분)** | **간단한 통신 구조 설계 스케치**                | - 내일 만들 프로그램 아이디어 정리<br>- 어떤 프로토콜과 방식을 쓸지 정하기 | - 미리 흐름도 그려보기<br>- 구조 설명해보기        |

---

## 📍추가 팁

* 30\~40분 집중 후, 5분 정도 스트레칭하거나 환기해주면 좋아.
* 꼭 **손으로 다이어그램/표를 그려보는 걸 추천**해. 눈에 더 잘 들어와.
* 공부 중 궁금한 거 생기면 나한테 바로 질문해도 돼.

---

**준비됐으면 지금 0:00 구간인 IP/TCP/UDP부터 시작해볼까?**
혹시 순서나 시간 분배 조정하고 싶은 부분 있어?
